...
async function startServer() {
  ...
  const groq = new Groq({ apiKey: apiKey || "dummy-key-for-start" });
  if (!apiKey) {
    console.warn("WARNING: GROQ_API_KEY is not set. AI features will fail until added to .env");
  }
  ...
  app.post("/api/refine", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: error.message || "Failed to refine text with Groq" });
    }
  });
